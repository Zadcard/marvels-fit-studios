...
export async function GET(
  ...
  const { data, error } = await getSupabaseServerClient()
    ...
    .select("id, receiptNumber, type, status, amount, currency, description, occurredAt, client:Client(fullName,userId), subscription:ClientSubscription(plan:SubscriptionPlan(name))")
    .eq("id", receiptId)
    .maybeSingle();
