...
| Categories & Groups (CRUD) | `app/(dashboard)/admin/categories/page.tsx` | `admin-training-categories-workspace.tsx` + `admin-groups-workspace.tsx` | `admin-training-category-repository.ts` + `admin-group-repository.ts` | `admin-training-categories.ts` + `admin-groups.ts` |
