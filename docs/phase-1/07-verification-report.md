...
| `npx supabase migration list --linked` | Pass; 4 local = 4 remote through `20260714000400`. |
| Later registration RPC pushes | Pass; migrations `20260714000300` and `20260714000400` applied without inserting application data. |
...
- Destination application row counts are intentionally zero because the user selected a fresh start.
- A linked database check on 2026-07-14 confirmed zero rows across all 19 application tables.
- Source comparison is intentionally not required; Neon data must not be imported.
...
- Neon data migration intentionally skipped by user decision; no Neon credential is required.
...
**Partially complete during the runtime cutover.** Repository cleanup, framework patching, reproducible empty Supabase schema, hosted schema application, RLS, typed client boundary, environment/deployment documentation, unit/build/E2E validation, and handoff are complete. Neon data migration is intentionally skipped. Runtime repository conversion, live role-flow verification, and full Prisma removal remain in progress.
