...
export function MarvelOpsToday({ data }: { data: AdminTodayOperations }) {
  ...
  return <div className={styles.page}>
    <section className={styles.metrics} aria-label="Today at a glance">
      <article><span>Live now</span><strong>{data.liveCount}<small>live</small></strong><p>{data.coaches.find((coach) => coach.status === "On floor")?.fullName ?? "No live session"}</p></article>
    ...
    <div className={styles.grid}>
      <div className={styles.primary}>
        <section className={styles.panel}>
          <header><div><span>OPERATIONS</span><h2>Today&apos;s sessions</h2><p>{dateLabel}</p></div><button type="button" onClick={() => router.push("/admin/schedule")}>Full schedule <ArrowRight size={15} /></button></header>
        ...
        <section className={styles.panel}>
          <header><div><span>COACHES</span><h2>Coaching right now</h2></div><button type="button" onClick={() => router.push("/admin/coaches")}>View coaches <ArrowRight size={15} /></button></header>
          <div className={styles.coaches}>{data.coaches.map((coach, index) => <button type="button" key={coach.id} onClick={() => router.push("/admin/coaches")}><i data-tone={tones[index % tones.length]}>{coach.initials}</i><span><b>{coach.fullName}</b><small>{coach.specialization}</small></span><em>{coach.detail}</em></button>)}{!data.coaches.length ? <p className={styles.panelEmpty}>No coaches in a live session right now.</p> : null}</div>
        </section>
      </div>
