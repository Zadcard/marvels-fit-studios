...
export function MarvelOpsCoachSchedule({ coachName, sessions }: { coachName: string; sessions: CoachSessionRecord[] }) {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.scheduleBoard}>
      ...
      <div className={styles.coachScheduleList}>{sessions.map((session) => <article key={session.id}><time>{session.timeLabel}</time><span className={styles.sessionTone} data-tone={session.sessionType === "Private" ? "violet" : "red"} /><div><strong>{session.title}</strong><small>{session.dayLabel}</small></div><b>{session.rosterLabel}</b><button type="button" onClick={() => router.push(`/coach/sessions?session=${encodeURIComponent(session.id)}`)}>Open session <ChevronRight size={14} /></button></article>)}{!sessions.length ? <p className={styles.coachScheduleEmpty}>No sessions scheduled this week.</p> : null}</div>
    </section>
  </div>;
}
