...
import { AlertTriangle, CalendarClock, ChevronLeft, ChevronRight, Clock3, MapPin, ShieldUser, Users, X } from "lucide-react";
...
export function MarvelOpsScheduleWorkspace({ sessions = [], weekOffset = 0 }: { sessions?: MarvelOpsScheduleSession[]; weekOffset?: number }) {
  ...
  function navigateWeek(nextOffset: number) {
    router.push(
      nextOffset === 0
        ? "/admin/schedule"
        : `/admin/schedule?week=${nextOffset}`,
    );
  }

  return <div className={styles.page}>
    <section className={styles.schedule} aria-label="Recurring schedule" style={{ display: "block" }}>
      <div className={styles.week}>
        <header className={styles.weekTitle}>
          ...
          <span><button type="button" aria-label="Previous week" onClick={() => navigateWeek(weekOffset - 1)}><ChevronLeft size={16} /></button><button type="button" aria-label="Next week" onClick={() => navigateWeek(weekOffset + 1)}><ChevronRight size={16} /></button></span>
        </header>
